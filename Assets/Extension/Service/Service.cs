...
public class Service : MonoBehaviour {
	...
	public class Formula : FormulaToolsService {
		...
		{
			return Json.AddSubPath(path, value);
		}
	...
	public class Time {
		...
		public class TimeServer
		{
			...
			public void Init(System.DateTime TimeServerNow) {
				...
				Debug.Log($"TimeServerNow {TimeServerNow} - DateTime.Now {System.DateTime.Now}");
			}
	...
	public class GameObj : MonoBehaviour {
		...
		public static void Refetch(GameObject root , float time = 0.05f )
		{
			var id = root.GetInstanceID().ToString();
			Timmer.StopAndDelete(id);
			root.SetActive(false);
			Timmer.Wait(time, PoolManager.pool.gameObject, () => {
				if (isObjectNotNull(root))
					root.SetActive(true);
			}).ID = id;
		}
	...
	public class Dict
	{
		...
		public static Service.Formula DictToFormula(Dictionary<string, int> dic)
		{
			var formula = new Service.Formula();
			foreach (var kvp in dic)
				formula.AddFormula(kvp.Key, kvp.Value);
			return formula;
		}
		public static Service.Formula DictToFormula(Dictionary<string, string> dic)
		{
		...
		public static Service.Formula DictToFormula(Dictionary<string, object> dic)
		{
			var formula = new Service.Formula();
			foreach (var kvp in dic)
				formula.AddFormula(kvp.Key, kvp.Value);
